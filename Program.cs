...
{
    public string ID { get; set; }      // Now writable
    public string SERVER { get; set; }  // Also writable

    public CREDENTIAL(string id, string serverInfo)
    {
        ID = id;
...
class Program
{
    //static readonly string BotToken = "7244935671:AAHQaqfMssQ3XLRrBAh1WOfCcAw7NNxQAW0";//YurriMarcoBot--103.246.144.125
    //static readonly string BotToken = "8291455039:AAGT24dMSqtubxj8oINS8PZLOuewBBwAKAI";//t.me/YurriMarcos2Bot---45.91.202.119
    static readonly string BotToken = "7679396013:AAFHAv0NRjQMk0QwWHog7Suw6LzUYdGwT_4";//t.me/YurriMarcos1Bot---212.86.115.100
    static readonly string MT5Path = @"C:\\Program Files\\MetaTrader 5\\terminal64.exe";
    static readonly string MT4Path = @"C:\\Program Files (x86)\\PU Prime MT4 Terminal\\terminal.exe";
    ...
    static void EnsureRdpSessionActive()
    {
        ...
        try
        {
            ...
            logger.Log($"PID-{currentSessionId} has been tscon applied!");
    ...
    private static async void Bot_OnMessage(object sender, MessageEventArgs e)
    {
        ...
        if (userVerificationStatus.ContainsKey(e.Message.Chat.Id) && userVerificationStatus[e.Message.Chat.Id])
        {
            if (e.Message.Text == "/start")
            {
                ...
                {
                    new[] { InlineKeyboardButton.WithCallbackData("🕹️ MetaTrader1", "metatrader1") },
                    new[] { InlineKeyboardButton.WithCallbackData("🕹️ MetaTrader2", "metatrader2") }
                });
            ...
            else if (e.Message?.Text?.StartsWith("/update") == true)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        // prevent MT1 credential equal to MT3 credential
                        if (selected_metatrader == 0)
                        {
                               if (cre_parts[0] == first_credential.ID)
                               {
                                    await bot.SendTextMessageAsync(e.Message.Chat.Id, "You are trying to login with same credential as the second Metatrader!");
                                    return;
                                }                               
                        }
                        else if (selected_metatrader ==1)
                        {
                            if (cre_parts[0] == second_credential.ID)
                            {
                                await bot.SendTextMessageAsync(e.Message.Chat.Id, "You are trying to login with same credential as the first Metatrader!");
                                return;
                            }
                        }
                        ...
                        {                            
                            ...
                            if (selected_metatrader == 0) {
                                first_credential.ID = cre_parts[0];
                                first_credential.SERVER = cre_parts[2];
                            }
                            else
                            {
                                second_credential.ID = cre_parts[0];
                                second_credential.SERVER = cre_parts[2];
                            }
                        ...
                        if (!success)
                        {
                            ...
                            if (success)
                            {
                                ...
                                {
                                    new[] { InlineKeyboardButton.WithCallbackData(" 🕹️ MetaTrader1", "metatrader1") },
                                    new[] { InlineKeyboardButton.WithCallbackData(" 🕹️MetaTrader2", "metatrader2") }
                                });
        ...
        else if (!userVerificationStatus[e.Message.Chat.Id])
        {
            ...
            {
                first_credential =  getCredential(0);
                second_credential = getCredential(1);
    ...
    public static CREDENTIAL getCredential(int selected_met)
    {
        var credential = new CREDENTIAL("", "");
            ...
            try
            {
                    ...
                    if (selected_met == 0)
                    {
                        ...
                        foreach (var p in existing)
                        {
                            if (p.MainWindowHandle != IntPtr.Zero)
                            {
                                ...
                                Console.WriteLine($"✅ Focused MT5 process (PID {p.Id})");
                                logger.Log($"✅ Focused MT5 process (PID {p.Id})");
                                proc = p;
                            }
                        }
                        if (existing.Length == 0)
                        {
                            proc = Process.Start(MT5Path);
                        }
                    }
                    else if (selected_met == 1)
                    {
                        var existing_4 = Process.GetProcessesByName("terminal"); // MT4 process name without .exe
                        foreach (var p in existing_4)
                        {
                            SetForegroundWindow(p.MainWindowHandle);
                            Console.WriteLine($"✅ Focused MT4 process (PID {p.Id})");
                            logger.Log($"✅ Focused MT4 process (PID {p.Id})");
                            proc = p;
                        }
                        if (existing_4.Length == 0)
                        {
                            proc = Process.Start(MT4Path);
                        }
                    }
                }
                catch (Exception ex)
                {
                    logger.Log("Failed to set MT5 to the foreground: " + ex.Message);
                    Console.WriteLine("Failed to set  MT5 to the foreground: " + ex.Message);
                    return credential;
                }
                ...
                {
                    logger.Log("Trying to find MT main window...");
                    Console.WriteLine("Trying to find MT main window...");

                    var window = app.GetMainWindow(automation);
                    if (window == null)
                    {
                        logger.Log("MT main window not found");
                        Console.WriteLine("MT main window not found");
                        return credential;
                    }
                    ...
                    if (selected_met == 0)
                    {
                        var match = Regex.Match(title, @"^(?<id>\d+)\s*-\s*(?<server>[^\s:]+)");
                        if (match.Success)
                        {
                            string accountId = match.Groups["id"].Value;
                            string serverName = match.Groups["server"].Value;

                            Console.WriteLine($"Account ID: {accountId}");
                            logger.Log($"Account ID: {accountId}");
                            Console.WriteLine($"Server Name: {serverName}");
                            logger.Log($"Account ID: {serverName}");
                            credential.ID = accountId;
                            credential.SERVER = serverName;
                        }
                        else
                        {
                            Console.WriteLine("Failed to parse account ID and server name.");
                        }
                    }
                    else if (selected_met == 1)
                    {
    ...
    private static async void Bot_OnCallbackQuery(object sender, CallbackQueryEventArgs e)
    {
            ...
            try
            {
                ...
                if (e.CallbackQuery.Data == "metatrader1")
                {
                    ...
                    responseMessage += $"\nCurrent Credential is ID:{first_credential.ID} Server:{first_credential.SERVER}";
                ...
                else if (e.CallbackQuery.Data == "metatrader2")
                {
                    ...
                    selected_metatrader = 1;
                    responseMessage += $"\nCurrent Credential is ID:{second_credential.ID} Server:{second_credential.SERVER}";
                    responseMessage += $"\nDo you want to change them? ";
                    var keyboard = new InlineKeyboardMarkup(new[]
                    {
                        new[] { InlineKeyboardButton.WithCallbackData("✅ Yes", "update_yes") },
    ...
    static void KillMT5IfRunning()
    {
        try
        {
            ...
            foreach (var p in existing_4)
            {
                p.Kill();
                p.WaitForExit();
                logger.Log($"Killed MT4 process (PID {p.Id})");
                Console.WriteLine($"Killed MT4 process (PID {p.Id})");
            }
        }
        catch (Exception ex)
        {
    ...
    static bool AutomateMT5(string login, string password, string server)
    {
        try
        {
            ...
            using (var automation = new UIA3Automation())
            {
                ...
                while (modalDialogs.Length > 0)
                {
                    logger.Log("number of dialogs" + modalDialogs.Length);
                    Console.WriteLine("number of dialogs" + modalDialogs.Length );
                    ...
                    modalDialogs = window.ModalWindows;
                    Thread.Sleep(1000);
                }

                Thread.Sleep(3000);
                var menuBar = selected_metatrader == 0 ? window.FindFirstDescendant(cf =>
                    cf.ByControlType(FlaUI.Core.Definitions.ControlType.MenuBar).And(cf.ByName("Application"))) : window.FindFirstChild(cf =>
                ...
                dlg.FindFirstDescendant(cf => cf.ByName("OK")).AsButton().Invoke();
                    ...
                    dlg.FindFirstDescendant(cf => cf.ByName("Login")).AsButton().Invoke();
                Thread.Sleep(2000);
                // Try finding the " Kb" text from main window directly
                int i = 0;
                bool connected =false;
                while (i<2){
                    if (selected_metatrader == 0)
                    {
                        // Find the Toolbox pane
                        var toolboxPane = window.FindFirstDescendant(cf => cf.ByName("Toolbox").And(cf.ByControlType(ControlType.Pane)));

                        if (toolboxPane == null)
                        {
                            Console.WriteLine("❌ Toolbox pane not found.");
                            return false;
                        }

                        // Find the VPS menu item
                        var vpsMenuItem = toolboxPane.FindFirstDescendant(cf =>
                            cf.ByName("VPS").And(cf.ByControlType(ControlType.MenuItem)));

                        var signalItem = toolboxPane.FindFirstDescendant(cf =>
                            cf.ByName("Signals").And(cf.ByControlType(ControlType.MenuItem)));

                        if (vpsMenuItem != null || signalItem!=null)
                        {
                            bool isEnabled = false;
                            if (vpsMenuItem != null)
                                isEnabled = vpsMenuItem.IsEnabled;
                            else
                                isEnabled = signalItem.IsEnabled;

                            Console.WriteLine($"VPS menu item is {(isEnabled ? "enabled" : "disabled")}");
                            connected = isEnabled;

                        }
                        else
                        {
                            Console.WriteLine("❌ VPS menu item not found.");
                            return false;
                        }

                        if (connected != true)
                        {
                            Console.WriteLine($"VPS not Enabled!");
                            menuBar = selected_metatrader==0? window.FindFirstDescendant(cf =>
                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.MenuBar).And(cf.ByName("Application"))): window.FindFirstChild(cf =>
                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.MenuBar).And(cf.ByName("Application")));
                            ...
                            var openAccountItem = window.FindFirstDescendant(cf =>
                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.MenuItem)
                                  .And(cf.ByName("Open an Account")))?.AsMenuItem();

                            if (openAccountItem == null)
                            {
                                openAccountItem = fileMenu.FindFirstDescendant(cf =>
                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.MenuItem)
                                  .And(cf.ByName("Open an Account")))?.AsMenuItem();

                                if (openAccountItem == null)
                                {
                                    logger.Log("❌ Open an Account item not found");
                                    Console.WriteLine("\u274C Open an Account item not found");
                                    return false;
                                }
                            }

                            openAccountItem.Click();
                            logger.Log("Open an Account Button Clicked!");
                            Console.WriteLine("Open an Account Button Clicked!");
                            Thread.Sleep(2000);

                            dlg = window.ModalWindows.FirstOrDefault();
                            if (dlg == null)
                            {
                                logger.Log("❌ Account dialog not found");
                                Console.WriteLine("\u274C Account dialog not found");
                                return false;
                            }

                            var dlg1 = window.ModalWindows.FirstOrDefault();
                            if (dlg1 == null)
                            {
                                logger.Log("❌ Input dialog not found");
                                Console.WriteLine("\u274C Input dialog not found");
                                return false;
                            }

                            // Try to find the first Edit box (regardless of its name)
                            var editBox = dlg1.FindFirstDescendant(cf => cf.ByControlType(FlaUI.Core.Definitions.ControlType.Edit))?.AsTextBox();

                            if (editBox == null)
                            {
                                logger.Log("❌ Edit box not found in dialog");
                                Console.WriteLine("\u274C Edit box not found in dialog");
                                return false;
                            }
                            ...
                            Thread.Sleep(1000);
                            var nextBtn = dlg.FindFirstDescendant(cf => cf.ByControlType(FlaUI.Core.Definitions.ControlType.Button).And(cf.ByName("Next >")));

                            if (nextBtn == null)
                            {
                                logger.Log("❌ Next Button not found in dialog");
                                Console.WriteLine("\u274C Next Button not found in dialog");
                                return false;
                            }
                            nextBtn.Click();
                            Thread.Sleep(1000);
                        ...
                        else
                        {
                            ...
                            var checkboxElement = window.FindFirstDescendant(cf =>
                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.CheckBox).And(cf.ByName("Algo Trading")));

                            if (checkboxElement == null)
                            {
                                Console.WriteLine("⚠️ 'Algo Trading' checkbox not found.");
                                return false;
                            }
                            ...
                            {
                                Console.WriteLine("✅ 'Algo Trading' is ENABLED.");
                                logger.Log("✅ 'Algo Trading' is ENABLED.");
                            }
                            else if (checkbox.IsChecked == false)
                            {
                                Console.WriteLine("❌ 'Algo Trading' is DISABLED.");
                                logger.Log("❌ 'Algo Trading' is DISABLED.");
                                checkbox.Click();
                            }
                            else
                            {
                                Console.WriteLine("☑️ 'Algo Trading' checkbox is in an indeterminate state.");
                                logger.Log("\"☑️ 'Algo Trading' checkbox is in an indeterminate state.");
                            }
                            return true;
                        }
                    }
                        ...
                        var statusBar = window.FindFirstDescendant(cf => cf.ByControlType(FlaUI.Core.Definitions.ControlType.StatusBar));

                        if (statusBar == null)
                        {
                            Console.WriteLine("Status bar not found.");
                            return false;
                        }

                        // Step 2: Search for any child element with text containing "kb"
                        var kbElement = statusBar.FindAllDescendants()
                                                 .FirstOrDefault(e => e.Name?.ToLower().Contains("kb") == true);

                        if (kbElement != null)
                        {
                            Console.WriteLine($"Found element with 'kb': {kbElement.Name}");
                            connected = true;
                            
                            var standardToolBar = window.FindFirstChild(cf =>
                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.ToolBar).And(cf.ByName("Standard")));

                            var checkboxElement = standardToolBar.FindFirstChild(cf =>
                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.CheckBox).And(cf.ByName("AutoTrading")));
                     

                            if (checkboxElement == null)
                            {
                                Console.WriteLine("⚠️ 'Auto Trading' checkbox not found.");
                                return false;
                            }
                            ...
                            if (checkbox.IsChecked == true)
                            {
                                Console.WriteLine("✅ 'Auto Trading' is ENABLED.");
                                logger.Log("✅ 'Auto Trading' is ENABLED.");
                            }
                            else if (checkbox.IsChecked == false)
                            {
                                Console.WriteLine("❌ 'Auto Trading' is DISABLED.");
                                logger.Log("❌ 'Auto Trading' is DISABLED.");
                                checkbox.Click();
                            }
                            else
                            {
                                Console.WriteLine("☑️ 'Auto Trading' checkbox is in an indeterminate state.");
                                logger.Log("\"☑️ 'Auto Trading' checkbox is in an indeterminate state.");
                            }
                            return true;
                        }
                        else
                        {
                            Console.WriteLine("No 'kb' element found in status bar.");
                            connected =false;
                        }
                        if (!connected)
                        {
                            ...
                            // Step 1: Find the "Open an Account" dialog
                            var openAccountDialog = window.FindFirstDescendant(cf =>
                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.Window)
                                  .And(cf.ByName("Open an Account")));

                            if (openAccountDialog == null)
                            {
                                Console.WriteLine("❌ 'Open an Account' dialog not found");
                                return false;
                            }

                            // Step 2: Find the unnamed child dialog inside "Open an Account"
                            //var innerDialog = openAccountDialog.FindFirstDescendant(cf =>
                            //    cf.ByControlType(FlaUI.Core.Definitions.ControlType.Window));

                            //if (innerDialog == null)
                            //{
                            //    Console.WriteLine("❌ Inner dialog (pane) not found inside 'Open an Account'");
                            //    return false;
                            //}

                            // Step 3: Find the "Scan" button inside the inner dialog
                            var scanButton = openAccountDialog.FindFirstDescendant(cf =>
                                cf.ByControlType(FlaUI.Core.Definitions.ControlType.Button)
                                  .And(cf.ByName("Scan")));

                            if (scanButton == null)
                            {
                                Console.WriteLine("❌ Scan button not found");
                                return false;
                            }
                            ...
                            Thread.Sleep(15000);
                            var closeBtn = openAccountDialog.FindFirstDescendant(cf =>
                                        cf.ByControlType(FlaUI.Core.Definitions.ControlType.Button)
                                          .And(cf.ByName("OK").Or(cf.ByName("Close")).Or(cf.ByName("Cancel"))));
                        ...
                        // Step 1: Find the "Standard" toolbar
